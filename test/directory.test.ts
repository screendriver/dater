...
test('getAllFiles() returns all files from given directory path', async () => {
  const actual = await getAllFiles('./test/assets');
  const expected = ['test-image-exif.jpg', 'test-image.jpg', 'text-only.txt'];
  expect(actual).toEqual(expected);
});

test('filterImages() filters all images from given files', () => {
...
test('resolveImagesFullPath() returns the absolute path of the given image', () => {
